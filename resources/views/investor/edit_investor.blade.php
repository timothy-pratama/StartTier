		...
		<div class="jumbotron">
			...
			<form action="{{route('edit_profil_investor', array('nama_investor'=>$nama_investor))}}" method="post" enctype="multipart/form-data">
				<div class="form-group">
					...
					<input type="text" class="form-control" name="nama_perusahaan" id="nama_perusahaan" value="{{$nama_investor}}" required>
				</div>
				<div class="form-group">
					<label for="alamat_perusahaan">Alamat Perusahaan</label>
					<textarea id="alamat_perusahaan" name="alamat_perusahaan" class="form-control" rows="10" style="resize: vertical" required>{{session('current_user')->alamat_perusahaan}}</textarea>
				</div>
				<div class="form-group">
					<label for="email_perusahaan">Email Perusahaan</label>
					<input type="email" class="form-control" id="email_perusahaan" name="email_perusahaan" value="{{session('current_user')->email}}" required>
				</div>
				<div class="form-group">
		...
		<div class="jumbotron">
			...
			<form action="{{route('edit_detil_investor', array('nama_investor'=>$nama_investor))}}" method="post" enctype="multipart/form-data">
				<div class="form-group">
					...
					<textarea class="form-control" rows="10" id="deskripsi_perusahaan" name="deskripsi_perusahaan" required>{{session('current_user')->deskripsi_perusahaan}}</textarea>
				</div>
				<div class="form-group">
